...
public class PointStrategyTestCase extends StrategyTestCase<PointFieldInfo> {
  ...
  public void testPointStrategy() throws IOException {
    ...
    PointStrategy s = new PointStrategy( new SimpleShapeIO(DistanceUnits.KILOMETERS),
        tinfo, FieldCache.NUMERIC_UTILS_DOUBLE_PARSER );

    if( false ) {
    // With JTS
    executeQueries( s, new JtsShapeIO(DistanceUnits.KILOMETERS), finfo,
        DATA_STATES_POLY,
        QTEST_States_IsWithin_BBox,
        QTEST_States_Intersects_BBox );

    // With SimpleIO
    executeQueries( s, new SimpleShapeIO(DistanceUnits.KILOMETERS), finfo,
        DATA_STATES_BBOX,
        QTEST_States_IsWithin_BBox,
        QTEST_States_Intersects_BBox );
    }
