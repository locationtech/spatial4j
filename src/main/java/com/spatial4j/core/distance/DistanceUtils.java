...
public class DistanceUtils {
   ...
   * @param distance The distance from the center to the corner
  ...
  public static double[] vectorBoxCorner(double[] center, double[] result, double distance, boolean upperRight) {
    ...
    // We don't care about the power here, b/c we are always in a rectangular coordinate system,
    // so any norm can be used by using the definition of sine
   ...
   * @param ctx SpatialContext for creating the resulting Point
   ...
   * results to avoid object creation. See http://janmatuschek.de/LatitudeLongitudeBoundingCoordinates Section 3.1, 3.2 and 3.3
   */
  public static Rectangle calcBoxByDistFromPtDEG(double lat, double lon, double distDEG, SpatialContext ctx, Rectangle reuse) {
    ...
    if (distDEG == 0) {
      ...
      if (maxY >= 90 || minY <= -90) { //touches either pole
        ...
        double lon_delta_deg = calcBoxByDistFromPtGetdeltaLonDEG(lat, lon, distDEG);
   ...
   * the bounding box of a circle. See http://gis.stackexchange.com/questions/19221/find-tangent-point-on-circle-furthest-east-or-west
   */
  public static double calcBoxByDistFromPtGetdeltaLonDEG(double lat, double lon, double distDEG) {
    if (distDEG == 0) {
      return 0;
    }

    double latRad = toRadians(lat);
    double distRad = toRadians(distDEG);
    double resultRad = Math.asin(Math.sin(distRad) / Math.cos(latRad));
    return !Double.isNaN(resultRad) ? toDegrees(resultRad) : 90;
  }
   ...
   * to be slightly different. See http://gis.stackexchange.com/questions/19221/find-tangent-point-on-circle-furthest-east-or-west
   */
  public static double calcBoxByDistFromPtGetlatHorizAxisDEG(double lat, double lon, double distDEG) {
    if (distDEG == 0) {
      return lat;
    }
    ...
    double latRad = toRadians(lat);
    double distRad = toRadians(distDEG);
    double resultRad = Math.asin( Math.sin(latRad) / Math.cos(distRad));
    if (!Double.isNaN(resultRad)) {
