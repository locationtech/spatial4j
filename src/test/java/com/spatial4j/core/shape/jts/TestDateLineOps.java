...
public class TestDateLineOps {
    ...
    public void testPolygon() throws ParseException {
        ...
        assertEquals("POLYGON ((-179 0, -181 0, -182 3, -179 0))", poly.toString());
    ...
    public void testSphericalRingAKAAntarctica() throws ParseException {
        ...
        assertEquals("POLYGON ((-180 -1, -180 0, -90 0, 0 0, 90 0, 180 0, 180 -1, -180 -1))", poly.toString());
    ...
    @Test
    public void testSegementizedSphericalRingAKAAntarctica() throws ParseException {
        double xmin = -180, xmax = 180;
    ...
    public void testShiftedPolygon() throws ParseException {
        ...
        assertEquals("POLYGON ((-539 0, -541 0, -542 3, -539 0))", poly.toString());
    ...
    public void testMultiPolygon() throws ParseException {
        ...
        Geometry geom = fromWKT("MULTIPOLYGON (((-179 0, 179 0, 178 3, -179 0)), " + nonsense + ")");
        Assert.assertEquals(unwrapDateline(geom, xmin, xmax), 1);
        assertEquals("MULTIPOLYGON (((-179 0, -181 0, -182 3, -179 0)), " + nonsense + ")", geom.toString());

        geom = DateLineOps.pageGeom(geom, xmin, xmax);
        assertEquals(
            "MULTIPOLYGON (((180 1, 180 0, 179 0, 178 3, 180 1)), ((-180 0, -180 1, -179 0, -180 0)), " + nonsense + ")",
            geom.toString());
    }
    ...
    public void testGeometryCollection() throws ParseException {
        ...
        String nonsense = "((1 0, 0 0, 0 1, 1 0))";
        Geometry geom = fromWKT("GEOMETRYCOLLECTION (POINT (0 0), MULTIPOLYGON (((-179 0, 179 0, 178 3, -179 0)), " + nonsense + "))");
        Assert.assertEquals(unwrapDateline(geom, xmin, xmax), 1);
        assertEquals("GEOMETRYCOLLECTION (POINT (0 0), MULTIPOLYGON (((-179 0, -181 0, -182 3, -179 0)), " + nonsense + "))", geom.toString());

        geom = DateLineOps.pageGeom(geom, xmin, xmax);
        assertEquals(
            "GEOMETRYCOLLECTION (POINT (0 0), MULTIPOLYGON (((180 1, 180 0, 179 0, 178 3, 180 1)), ((-180 0, -180 1, -179 0, -180 0)), " + nonsense + "))",
            geom.toString());
    }
    ...
    public void testNegativePage() {
        assertEquals(
            ...
            DateLineOps.pageGeom(
                F.toGeometry(new Envelope(-181, -179, 0, 1)),
                -180,
                180)
                .toString());
    }
    ...
    public void testCollectNestedGeometryCollection() {
        ...
        Geometry gc = f.createGeometryCollection(new Geometry[]{box(0, 0, 1, 1)});
    ...
    private static Polygon box(double xmin, double ymin, double xmax, double ymax) {
