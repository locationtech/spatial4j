...
public class GeoHashDistanceFilter extends DistanceFilter {
   ...
   * @param miles
   */
  public GeoHashDistanceFilter(Filter startingFilter, ShapeIO shapeIO, double lat, double lng, double miles, String geoHashField) {
  ...
  public DocIdSet getDocIdSet(AtomicReaderContext context) throws IOException {
    ...
    return new FilteredDocIdSet(startingFilter.getDocIdSet(context)) {
      ...
      public boolean match(int doc) {
        ...
        double[] coords = GeoHashUtils.decode(geoHash,shapeIO);
        double x = coords[0];
        double y = coords[1];
